...
public enum TypeConstruction {
    ...
    >(
        ...
        //     int_type Data;
        //     ...
        let elementCountFlag = 0x0000_FFFF
        assert(
            elementTypes.count != 1,
            "A one-element tuple is not a realistic Swift type")
        assert(
            elementTypes.count <= elementCountFlag,
            "Tuple size exceeded \(elementCountFlag)")
        ...
        let result = elementTypes.withContiguousStorageIfAvailable {
    ...
    public static func tuple<Elements: BidirectionalCollection>(
        ...
        func create<T>(_: T.Type) -> Any {
            ...
            var currentElementAddressUnaligned = UnsafeMutableRawPointer(
                baseAddress)
            for element in elements {
                // Open existential on each element type.
                func initializeElement<U>(_ element: U) {
                    currentElementAddressUnaligned =
                        currentElementAddressUnaligned.roundedUp(
                            toAlignmentOf: U.self)
                    ...
                    currentElementAddressUnaligned =
                        currentElementAddressUnaligned.advanced(
                            by: MemoryLayout<U>.size)
                }
